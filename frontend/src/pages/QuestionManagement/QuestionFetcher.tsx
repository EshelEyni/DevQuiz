...
export const QuestionFetcher: FC = () => {
  ...
  return (
    <div className="flex w-full flex-col gap-4 rounded-lg">
      <div className="flex w-full flex-wrap items-center justify-between gap-4 border-b-2 border-gray-950 bg-gray-800 p-4">
        <div className="flex flex-wrap items-center gap-4">
          <textarea
            className="h-24 w-[350px] rounded-xl bg-gray-700 p-5 text-4xl leading-5 text-gray-100 outline-none"
          ...
          <Select onChange={handleChangeLangSelect}>
            <Select.SelectTrigger
              className="h-24 w-52 cursor-pointer rounded-xl 
            bg-gray-700 text-3xl leading-5 outline-none transition-all duration-300"
            >
              <button type="button">{formtState.language}</button>
            </Select.SelectTrigger>
            <Select.SelectList
              className="z-[1500] mt-1 w-52  min-w-[100px] cursor-pointer 
            border-2  border-gray-800 bg-gray-700 text-3xl leading-5 outline-none transition-all duration-300"
            >
              {Object.keys(programmingLanguages).map((lang: string) => (
                <Select.SelectItem
                  key={lang}
                  value={lang}
                  className="flex h-24 w-full min-w-[100px]  cursor-pointer items-center justify-center border-b-2 border-gray-800 bg-gray-700 text-3xl
          ...
          <Select onChange={handleChangeDifficultySelect}>
            <Select.SelectTrigger
              className="h-24 w-52 cursor-pointer rounded-xl 
            bg-gray-700 text-3xl capitalize leading-5 outline-none transition-all duration-300"
            >
            ...
            >
              {difficultyLevels.map((lang: string) => (
                <Select.SelectItem
                  key={lang}
                  value={lang}
                  className="flex h-24 w-full min-w-[100px]  cursor-pointer items-center justify-center border-b-2 border-gray-800 bg-gray-700 text-2xl
                    text-gray-100 hover:border-gray-900 hover:bg-gray-950 hover:text-gray-100"
                >
